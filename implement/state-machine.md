...
> **归档参考**：状态机的完整原文存档。权威源为 `core/manifest.md`。`tools.md` A.0 矩阵和 `README.md` 核心流程均派生自 manifest。修改状态必须先改 manifest，再同步此归档。
...
READ_CONTEXT
  ...
  -> COMPLETE（当前请求完成且无 pending milestone）
```
...
| `PLAN_STEP` | 当前 milestone 的 AC、Reality Links 和验证命令 | 拆成本轮可完成的工作单元，每个单元标注：**id / depends_on / scope / acceptance_tests / risk_level / rollback_plan**。按依赖关系排 DAG 顺序，互不依赖的标记为可并行。复杂度分级：Tier 1（单文件、确定性测试）/ Tier 2（多文件行为变更）/ Tier 3（schema/auth/perf 变更）。**>2 个新文件或涉及新模块时，必须先画出模块边界图** | 有清晰工作单元列表 + 依赖图 + 复杂度标注；若 AC/命令/现实链路缺失则补计划或进入 `BLOCK` |
...
| `REPAIR` | 失败日志 | 定位并修复失败；最多连续修复 3 轮；不得通过删除关键断言、跳过真实场景或迎合 evaluator 形式项来伪修复。**卡壳恢复**：同一问题修 2 次仍失败 → 不继续硬修：evict 当前方案 → snapshot 已发现的问题和尝试过的路径 → 收窄范围重新定义最小可验证步骤 → 重试 | 修复后回到 `VALIDATE`；仍失败进入 `BLOCK`（附带 snapshot） |
...
| `COMPLETE` | 当前请求完成，无 pending milestone | 输出最终交付总结（D.6.6 格式）；更新 Documentation.md §1/§7/§10 | 停止，等用户下一个请求 |

#### 规则弹性：按改动规模分级应用

刚性规则在微小改动中成本过高。以下分级定义何时可以简化、何时必须严守：

| 改动规模 | 判定标准 | TDD | AI 自审 | 需求门禁 | 交付总结 |
|----------|----------|-----|---------|----------|----------|
| **Trivial** | < 5 行，typo/格式化/注释 | 跳过 | 跳过 | 跳过 | 跳过 |
| **Small** | < 50 行，单文件，纯逻辑 | 补一个关键断言即可 | 自检代替 | 跳过 | 一行 heartbeat |
| **Medium** | 多文件，行为变更 | 完整红→绿→重构 | **必调** code-reviewer | 设计方案需确认 | 标准交付总结 |
| **Large** | 新功能，schema 变更，> 5 文件 | 完整 TDD + 集成测试 | **必调** code-reviewer + security-reviewer | **硬门禁**：2-3 方案批准后实现 | 完整 D.6.6 交付总结 |

**铁律不变**：任何豁免必须在 RECORD 时写明原因。连续跳过 = 空转信号。
...
2. **启动并行 agents**：用 `Skill(dispatching-parallel-agents)` 并发分派独立子任务
...
1. **依赖检查**：不合并有未解决依赖失败的单元——先修依赖，再合
2. **Rebase**：每个单元分支在合并前 rebase 到最新集成分支
3. **集成测试**：每次入队合并后重跑集成测试——合并顺序可能导致意外冲突
4. **冲突处理**：两个 agent 改了同一文件的同一区域 → 手动合并，合并后重跑该单元的审查

### D.2 停止与阻塞条件

以下 6 种模式触发 `BLOCK`，停止当前执行。不是列表越大越安全——是信号越清晰越安全。
