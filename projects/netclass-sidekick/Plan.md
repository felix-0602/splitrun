...
### Test Evidence（必填——无测试不得 ADVANCE）
- [ ] 新增测试文件：[列出文件路径]
- [ ] 覆盖的关键路径：
  - [ ] 新增/修改的函数 A：成功路径 + ≥1 失败路径
  - [ ] 新增/修改的函数 B：成功路径 + ≥1 失败路径
- [ ] 测试在本 milestone 已执行通过：`[命令 + 输出摘要]`

> 豁免条件（仅在以下情况可填"无"）：纯配置变更 / 纯文档修改 / 纯 UI 样式微调。豁免需在 Residual Risks 中记录。
