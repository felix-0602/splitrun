...
### 临时验证脚本（.claude/DEEPSHIP/checks/）

项目 `.claude/DEEPSHIP/checks/` 是一次性验证脚本的暂存区：

**为什么不会污染项目**：`.claude/` 整个目录在 `.gitignore` 里——checks/ 里的任何东西都不会进入版本控制或 `git status`。

| 什么时候用 | 什么时候不用 |
|------------|-------------|
| 验证 HTML fixture 结构、API 响应字段 | 正式测试——用 A.3.1 的测试框架 |
| 跨文件引用完整性的一次性扫描 | 可复用的测试逻辑——移入项目测试套件 |
| 部署后冒烟检查的临时脚本 | 需要版本控制的验证——放项目 test/ 目录 |
| 项目特有的格式/结构检查 | 框架自检——那是框架自己 checks/ 的事 |

**生命周期**：
1. **写**：在 checks/ 里创建脚本，跑通
2. **验**：确认结果，记录到 Documentation.md（如有需要）
3. **删**：跑完就删。不复用的不进测试套件，不累积历史遗物

**样例**：见 `.claude/DEEPSHIP/checks/sample-api-check.py`。

**铁律**：跑通后删掉。留下来的要么是永久项目检查（如 verify.py），要么是垃圾（删掉）。没有"暂时留着以后可能有用"。
