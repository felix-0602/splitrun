...
```
/splitrun-scope   先对齐任务认知——"我理解对了吗？值不值得并行？"
/splitrun-spawn   拆 WU，开隔离 worktree，并行启动 Claude Code 会话
/splitrun-status  看所有 Lane 状态——谁 done、谁 blocked、能不能 land
/splitrun-land    收报告、验边界、跑测试、合并分支、出交付摘要
```
...
- **scope** 让 Brain 在动手前把认知摊开给你纠偏。产物是 `.splitrun/scope.md`
- **spawn** 基于共识拆 WU，展示文件边界，你确认后开 Lane。调 `spawn_lane.py` 创建 worktree + 启动终端
- **status** 聚合视图——done/blocked/越界/无报告/can-land 判定
- **land** 三类检查（Boundary/Evidence/Integration）全过才 merge。产物是 `.splitrun/land-report.md`
...
│   ├── splitrun-status/SKILL.md
│   └── splitrun-land/SKILL.md
├── adapters/
│   ├── brain/                  # WU 分派 + Lane 监控
│   │   ├── dispatch.py
│   │   └── monitor.py
